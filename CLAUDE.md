...
twelve inline aesthetics), and **`src/app-*.js` — 31 ordered classic `<script>`s** holding all
...
long after all 31 files have loaded. What order *does* constrain is anything that runs *while a
...
new work, not a restore. `npm test` is currently **99/99 test files passing** (`run_all.js`

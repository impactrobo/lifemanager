...
twelve inline aesthetics), and **`src/app-*.js` — 22 ordered classic `<script>`s** holding all
...
long after all 22 files have loaded. What order *does* constrain is anything that runs *while a
...
new work, not a restore. `npm test` is currently **69/69 test files passing** (`run_all.js`

...
twelve inline aesthetics), and **`src/app-*.js` — 32 ordered classic `<script>`s** holding all
...
long after all 32 files have loaded. What order *does* constrain is anything that runs *while a
...
new work, not a restore. `npm test` is currently **102/102 test files passing** (`run_all.js`

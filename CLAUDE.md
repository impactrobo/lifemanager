...
twelve inline aesthetics), and **`src/app-*.js` — 24 ordered classic `<script>`s** holding all
...
long after all 24 files have loaded. What order *does* constrain is anything that runs *while a
...
new work, not a restore. `npm test` is currently **71/71 test files passing** (`run_all.js`

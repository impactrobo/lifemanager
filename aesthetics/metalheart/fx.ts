/**
 * Metalheart — cable-field parallax.
 *
 * Unlike the other FX modules this one draws nothing. It only writes two CSS custom properties,
 * `--mh-px` / `--mh-py`, which theme.css feeds into a `translate3d()` on the fixed cable layer.
 * Doing it that way means the cable field still renders (just static) when this module isn't
 * loaded at all — no JS, or a `file://` page where ES modules can't be imported — and the
 * motion stays a compositor-only transform rather than anything repainting.
 *
 ...
 *  - `destroy()` releases the rAF handle and every listener, and clears the two properties so
 *    the next theme can't inherit an offset.
...
/** Furthest the field travels from centre, px. Small on purpose — it should register as depth,
...
function createMetalheartFX(): AestheticFX {
  let rafId = 0;
  let targetX = 0;
  let targetY = 0;
  let curX = 0;
  ...
  function write(): void {
    if (!root) return;
    root.style.setProperty('--mh-px', curX.toFixed(2) + 'px');
    root.style.setProperty('--mh-py', curY.toFixed(2) + 'px');
  }
  ...
  /** Scroll drives the vertical component: the field lags the page, which reads as depth. */
   ...
   *  during a drag, which would make the field twitch mid-scroll. */
  ...
  return {
    ...
    destroy(): void {
      ...
      if (root) {
        root.style.removeProperty('--mh-px');
        root.style.removeProperty('--mh-py');
      }
      root = null;
      reduceMotion = null;
...
export default createMetalheartFX();

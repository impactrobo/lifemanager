...
function createCartomancerFX() {
    ...
    function spawnMote(startBelowFold) {
    ...
    function refreshColorIfStale(dt) {
        colorAge += dt;
        if (colorAge < COLOR_REFRESH_MS)
            return;
        colorAge = 0;
    ...
    function start() {
        ...
        if (reduceMotion && reduceMotion.matches)
            return; // no particles at all, same as Draconic

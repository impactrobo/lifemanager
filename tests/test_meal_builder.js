...
(async () => {
  ...
  const draftAfterStart = await page.evaluate(() => VIEW.mealBuilderDraft && { name: VIEW.mealBuilderDraft.name, items: VIEW.mealBuilderDraft.items.length });
  console.log('draft after startNewMeal():', draftAfterStart);
  if (!draftAfterStart || draftAfterStart.items !== 0) throw new Error('Expected a fresh empty VIEW.mealBuilderDraft after startNewMeal()');
  ...
  const draftAfterAdd = await page.evaluate(() => ({ name: VIEW.mealBuilderDraft.name, items: VIEW.mealBuilderDraft.items }));
  ...
  const qtyAfterEdit = await page.evaluate((id) => VIEW.mealBuilderDraft.items.find(it => it.id === id).qty, itemId);
  ...
  const afterSwitch = await page.evaluate((id) => VIEW.mealBuilderDraft.items.find(it => it.id === id), itemId);
  ...
  const draftAfterSave = await page.evaluate(() => VIEW.mealBuilderDraft);
  ...
  if (draftAfterSave !== null) throw new Error('Expected VIEW.mealBuilderDraft to clear to null after saving');
  ...
  const draftStillOpenAfterEmptySave = await page.evaluate(() => VIEW.mealBuilderDraft !== null);
  ...
  const reopened = await page.evaluate(() => ({ id: VIEW.mealBuilderDraft.id, name: VIEW.mealBuilderDraft.name, items: VIEW.mealBuilderDraft.items.length }));

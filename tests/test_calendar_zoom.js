...
// own tap-to-zoom-out breadcrumb behavior.
...
(async () => {
  ...
  // 1. Defaults to Month zoom (unchanged existing behavior — test_calendar.js depends on this too)
  const initialZoom = await page.evaluate(() => CAL_ZOOM);
  console.log('initial CAL_ZOOM:', initialZoom);
  if (initialZoom !== 'month') throw new Error(`Expected default CAL_ZOOM "month", got "${initialZoom}"`);
  const monthGridVisible = await page.evaluate(() => document.querySelectorAll('.cal-grid .cal-cell:not(.cal-cell-blank)').length > 25);
  if (!monthGridVisible) throw new Error('Expected a full month grid to render by default');
  ...
  // 8. jumpToReminderDay() always lands on Month zoom regardless of whatever zoom was last active
  await page.evaluate(() => calSetZoom('year'));
  ...
  if (afterJump.zoom !== 'month') throw new Error(`Expected jumpToReminderDay() to reset zoom to "month", got "${afterJump.zoom}"`);
  if (afterJump.selected !== targetDate) throw new Error('Expected jumpToReminderDay() to select the target date');
